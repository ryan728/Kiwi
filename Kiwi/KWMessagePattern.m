    ...
    for (NSUInteger i = 0; i < numberOfMessageArguments && i < numberOfArgumentFilters; ++i) {
        ...
        if ([argumentFilter conformsToProtocol:@protocol(HCMatcher)]) {
            ...
            if (!KWObjCTypeIsPointerLike(objCType)) {
                [NSException raise:@"KWMessagePatternException" format:@"nil was specified as an argument filter, but argument(%lu) is not a pointer for @selector(%@)", i + 1, NSStringFromSelector([anInvocation selector])];
            }
            void *p = nil;
